...
export interface MailingsFilters {
  search: string;
  type: string;
  status: string;
  dateFrom: string;
...
const update = () => emit("update:filters", { ...localFilters.value });

const blurTarget = (e: Event) => (e.target as HTMLElement).blur();

const reset = () => {
  localFilters.value = { search: "", type: "all", status: "all", dateFrom: "", dateTo: "" };
  ...
  <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    ...
    <div class="px-3 py-2.5 flex items-center gap-2">
      ...
      <div class="relative flex-1 min-w-0">
        <AppIcon name="search" :size="14" class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
        <input
      ...
      <!-- Кнопка доп. фильтров — только мобайл -->
      <button
      ...
      >
        ...
        <span v-if="hasExtraActive" class="absolute -top-1 -right-1 w-2 h-2 bg-blue-500 rounded-full" />
      </button>
      ...
      <select
        ...
        @change="update"
        class="hidden md:block px-2.5 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white shrink-0"
      >
        <option v-for="t in types" :key="t.value" :value="t.value">{{ t.label }}</option>
      </select>

      <!-- Даты — только десктоп -->
      <input v-model="localFilters.dateFrom" @change="update" type="date"
      ...
      <input v-model="localFilters.dateTo" @change="update" type="date"
        class="hidden md:block px-2.5 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />

      <!-- Сбросить — только десктоп -->
      <button
    ...
    <Transition name="mf-expand">
      <div v-if="showMore" class="md:hidden border-t border-gray-100 px-3 pb-3 pt-2.5 space-y-2">
...
<style scoped>
.mf-expand-enter-active { transition: all 180ms ease-out; overflow: hidden; }
.mf-expand-leave-active { transition: all 140ms ease-in;  overflow: hidden; }
.mf-expand-enter-from,
.mf-expand-leave-to     { opacity: 0; max-height: 0; }
.mf-expand-enter-to,
.mf-expand-leave-from   { opacity: 1; max-height: 300px; }
</style>
