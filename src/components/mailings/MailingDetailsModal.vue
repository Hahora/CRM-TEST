...
const startEditing = () => {
  ...
  editForm.value = {
    name: props.mailing.name,
    type: props.mailing.type,
    message: props.mailing.message,
    scheduledAt: props.mailing.scheduledAt
      ? utcToMoscowLocal(props.mailing.scheduledAt)
      : "",
    sendNow: false,
  };
  ...
  setTimeout(() => {
    if (editorRef.value)
      editorRef.value.innerHTML = tgMdToHtml(props.mailing!.message);
  }, 50);
};
...
const fmtButtons = [
  {
    ...
    title: "Жирный",
    cls: "fw-bold",
    action: () => applyFmt("bold"),
  },
  {
    label: "I",
    title: "Курсив",
    cls: "fw-italic",
    action: () => applyFmt("italic"),
  },
  {
    label: "U",
    title: "Подчёркнутый",
    cls: "fw-under",
    action: () => applyFmt("underline"),
  },
  {
    label: "S",
    title: "Зачёркнутый",
    cls: "fw-strike",
    action: () => applyFmt("strikeThrough"),
  },
  {
    label: "< >",
    title: "Код",
    cls: "fw-code",
    action: () => toggleInlineTag("code"),
  },
  {
    label: "||",
    title: "Спойлер",
    cls: "fw-spoil",
    action: () => toggleInlineTag("span", "tg-spoiler"),
  },
];
  ...
  const walk = (node: Node): string => {
    ...
    switch (tag) {
      ...
      case "code":
        return `\`${ch}\``;
      case "span":
        return el.classList.contains("tg-spoiler") ? `||${ch}||` : ch;
      case "br":
        return "\n";
      case "div":
      case "p":
...
const handleSave = async () => {
  ...
  if (!editForm.value.sendNow && editForm.value.scheduledAt) {
    if (
      new Date(editForm.value.scheduledAt + ":00+03:00").getTime() <= Date.now()
    ) {
  ...
  try {
    ...
    if (m.templateId) {
      ...
      await mailingsApi.updateTemplate(m.templateId, {
        ...
        content,
        bot_type: editForm.value.type,
        ...(mediaUrl
          ? { media_url: mediaUrl, media_type: mediaType }
          : clearExistingMedia.value
          ? { media_url: null, media_type: null }
          : {}),
      });
    }
    submitStep.value = "Сохранение...";
    const scheduledUtc = editForm.value.scheduledAt
      ? moscowLocalToUtc(editForm.value.scheduledAt)
      : null;
...
const getType = (t: string) =>
  t === "telegram"
    ? { label: "Telegram", cls: "text-blue-600 bg-blue-50" }
    : { label: "МАКС", cls: "text-purple-600 bg-purple-50" };
...
const handleOverlay = (e: MouseEvent) => {
  if (
    (e.target as HTMLElement).classList.contains("dm-overlay") &&
    !isSubmitting.value &&
    !isSending.value
  )
    emit("close");
...
const getMoscowNow = (): string =>
  new Intl.DateTimeFormat("sv-SE", _mosOpts)
    .format(new Date())
    .replace(" ", "T");
const utcToMoscowLocal = (iso: string): string =>
  new Intl.DateTimeFormat("sv-SE", _mosOpts)
    .format(new Date(iso))
    .replace(" ", "T");
const moscowLocalToUtc = (local: string): string =>
      ...
      <div v-if="isOpen && mailing" class="dm-overlay" @click="handleOverlay">
        <Transition name="dm-slide">
          <div v-if="isOpen && mailing" class="dm-panel">
            ...
            <div class="dm-header">
              <div class="dm-header-left">
                ...
                <div class="dm-header-info">
                  <h2 class="dm-title">
                    {{ isEditing ? "Редактировать черновик" : mailing.name }}
                  </h2>
                  <div class="dm-badges">
                    <span class="dm-type-label">{{
                      getType(mailing.type).label
                    }}</span>
                    <span
                      class="dm-status-badge"
            ...
            <div class="dm-body">
              ...
              <template v-if="!isEditing">
                <div class="dm-section">
                  ...
                  <div
                    class="dm-message"
                    v-html="tgMdToHtml(mailing.message || '—')"
                  />
                ...
                <div class="dm-section">
                  ...
                  <div v-if="statsLoad" class="dm-stats-loading">
                    <AppIcon name="refresh-cw" :size="13" class="dm-spin" />
                    Загрузка...
                  </div>
                  <div v-else-if="!stats" class="dm-stats-empty">
                  ...
                  <div v-else>
                    <div class="dm-stats-grid">
                      <div class="dm-stat dm-stat--blue">
                        ...
                        <div class="dm-stat-label">Всего</div>
                      </div>
                      <div class="dm-stat dm-stat--green">
                        <div class="dm-stat-value">
                          {{ stats.sent_messages.toLocaleString() }}
                        </div>
                ...
                <div class="dm-section">
                  ...
                  <div class="dm-info-list">
                    <div class="dm-info-row">
                      <span class="dm-info-label">Создано</span
                      ><span class="dm-info-value">{{
                        fmt(mailing.createdAt)
                      }}</span>
                    </div>
                    ...
                    <div v-if="mailing.sentAt" class="dm-info-row">
                      ...
                      ><span class="dm-info-value dm-info-value--green">{{
                        fmt(mailing.sentAt)
                      }}</span>
                    </div>
                    <div v-if="mailing.recipients > 0" class="dm-info-row">
                      <span class="dm-info-label">Получателей</span
                      ><span class="dm-info-value">{{
              ...
              <template v-else>
                <form @submit.prevent="handleSave" class="dm-form">
                  <div class="dm-section">
                    ...
                    <div class="dm-fields">
                      <div class="dm-field dm-field--req">
                      ...
                      <div class="dm-field">
                        ...
                        <div class="dm-types">
                          <button
                            ...
                            class="dm-type"
                            :class="{
                              'dm-type--on': editForm.type === t.value,
                            }"
                  ...
                  <div class="dm-section">
                    <div class="dm-section-row">
                      <h3 class="dm-section-title">
                        Текст сообщения
                        <span v-if="charCount > 0" class="dm-count"
                          >{{ charCount }} симв.</span
                        >
                      </h3>
                      <div class="relative">
                    ...
                    <div class="dm-fmt-bar">
                      ...
                      <button
                        ...
                        class="dm-fmt-btn dm-fmt-btn--clear"
                        @mousedown.prevent
                        @click="clearFormat"
                      >
                        <AppIcon name="x" :size="11" /><span>Сброс</span>
                      </button>
                    </div>
                    <div
                      ref="editorRef"
                  ...
                  <div class="dm-section">
                    <h3 class="dm-section-title">
                      Медиа
                      <span v-if="mediaFiles.length" class="dm-count">{{
                        mediaFiles.length
                      }}</span>
                    </h3>
                    <input
                    ...
                    <div
                      class="dm-dropzone"
                      :class="{ 'dm-dropzone--drag': isDragging }"
                      @click="mediaInputRef?.click()"
                      @dragover.prevent="isDragging = true"
                      @dragleave.prevent="isDragging = false"
                      @drop.prevent="onDrop"
                    >
                      <AppIcon name="paperclip" :size="16" /><span
                        >Перетащите или <u>выберите файлы</u></span
                      ><span class="dm-dropzone-sub"
                        >Фото, видео, документы</span
                      >
                    </div>
                    <div
                      v-if="
                        ...
                        !clearExistingMedia
                      "
                      class="dm-existing-media"
                    >
                      <AppIcon
                        :name="mediaIcon(mailing.mediaType)"
                        :size="13"
                      />
                      <span
                        >Прикреплено:
                      ...
                      <button
                        type="button"
                        class="dm-existing-media-rm"
                        title="Убрать медиа"
                        @click="clearExistingMedia = true"
                      >
                    ...
                    <div v-if="mediaFiles.length" class="dm-media-list">
                      ...
                      >
                        <img
                          ...
                          :src="mediaPreviews[i]"
                          class="dm-media-thumb"
                          :alt="f.name"
                        />
                        <div v-else class="dm-media-icon">
                          <AppIcon :name="fileIcon(f.type)" :size="15" /><span
                            class="dm-media-fname"
                            >{{ f.name }}</span
                          ><span class="dm-media-sz">{{
                            formatBytes(f.size)
                          }}</span>
                        </div>
                        <button
                    ...
                    <div class="dm-fields">
                      <div class="dm-field">
                        <label class="dm-check-label"
                          ><input
                            type="checkbox"
                            v-model="editForm.sendNow"
                          /><span>Отправить сейчас</span></label
                        >
                      </div>
                      <div v-if="!editForm.sendNow" class="dm-field">
                        <label
                          >Запланировать
                          <span class="dm-optional">необязательно</span></label
                        >
                        <input
                        ...
                        />
                        <span class="dm-hint"
                          >Оставьте пустым — сохранится как черновик</span
                        >
                      </div>
                    </div>
                  </div>
            ...
            <div class="dm-footer">
              <template v-if="!isEditing">
                <div v-if="sendError" class="dm-footer-error">
                  <AppIcon name="alert-circle" :size="13" /> {{ sendError }}
                </div>
                <button
              ...
              <template v-else>
                <div v-if="submitError" class="dm-footer-error">
                  <AppIcon name="alert-circle" :size="13" /> {{ submitError }}
                </div>
                <button
                  class="dm-btn dm-btn--ghost"
...
.dm-title {
  font: 700 15px/1.2 var(--fn, sans-serif);
  color: #0f172a;
  margin: 0;
  white-space: nowrap;
  overflow: hidden;
...
.dm-btn-close:hover:not(:disabled) {
  background: #f1f5f9;
  color: #0f172a;
}
.dm-btn-close:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
...
.dm-section-title {
  font: 700 10px/1 var(--fn, sans-serif);
  color: #94a3b8;
  margin: 0 0 12px;
...
.dm-section-row .dm-section-title {
  margin-bottom: 0;
}
.dm-count {
...
.dm-stat-value {
  font: 700 18px/1 var(--fn, sans-serif);
}
.dm-stat--blue .dm-stat-value {
  color: #2563eb;
}
.dm-stat--green .dm-stat-value {
  color: #16a34a;
}
.dm-stat--purple .dm-stat-value {
  color: #7c3aed;
}
.dm-stat--amber .dm-stat-value {
  color: #d97706;
}
.dm-stat-label {
  font: 500 10px/1 var(--fn, sans-serif);
...
.dm-check-label input[type="checkbox"] {
  width: 16px;
  height: 16px;
  accent-color: #2563eb;
  cursor: pointer;
...
.dm-media-sz {
  font: 400 9px/1 var(--fn, sans-serif);
  color: #94a3b8;
}
.dm-media-rm {
  position: absolute;
...
.dm-media-item:hover .dm-media-rm {
  opacity: 1;
}
.dm-media-rm:hover {
  background: #dc2626;
