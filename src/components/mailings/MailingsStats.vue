...
<template>
  <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-3">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 bg-slate-50 text-slate-500 rounded-lg flex items-center justify-center flex-shrink-0">
          <AppIcon name="x-circle" :size="16" />
        </div>
        <div class="min-w-0">
          <div class="text-xl font-bold text-gray-900 leading-none">{{ stats.cancelled }}</div>
          <div class="text-xs text-gray-500 mt-0.5">Отменено</div>
        </div>
      </div>
    </div>
  </div>
