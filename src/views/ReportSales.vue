...
const handleDownload = async () => {
  ...
  downloadError.value = "";
  try {
    const result = await reportsApi.downloadSales({
      start_date: startDate.value || undefined,
      end_date: endDate.value || undefined,
      branch_ids: selectedBranchId.value !== null ? [selectedBranchId.value] : undefined,
    });
...
onMounted(async () => {
  try {
    ...
    if (isBranch.value && user.value?.branch_id != null) {
      ...
      selectedBranchId.value = branches.value[0]?.local_id ?? null;
    } else {
...
<template>
  <div class="rep-page" style="--rep-color:#d97706;--rep-light:#fffbeb;--rep-mid:#fde68a;">
    ...
    <div class="rep-strip">
      ...
      <div class="rep-title-row">
        ...
        <div>
          <h1 class="rep-title">Отчет по продажам</h1>
          <p class="rep-subtitle">Чеки, суммы, товары, консультанты, магазины</p>
        </div>
      </div>
    </div>
...
.rep-page {
  ...
  background: #f8fafc; font-family: "Manrope", -apple-system, BlinkMacSystemFont, sans-serif; color: #0f172a;
}
.rep-strip { background: var(--rep-light); border-bottom: 1.5px solid var(--rep-mid); padding: 18px 24px 22px; flex-shrink: 0; }
.rep-back {
  display: inline-flex; align-items: center; gap: 5px; background: none; border: none;
  color: var(--rep-color); font: 600 12px "Manrope", sans-serif; cursor: pointer;
  padding: 0; opacity: 0.75; transition: opacity 120ms; margin-bottom: 14px;
}
...
.rep-icon {
  ...
  justify-content: center; color: var(--rep-color); flex-shrink: 0;
}
...
.rep-section-label {
  ...
  font: 700 10px/1 "Manrope", sans-serif; text-transform: uppercase; letter-spacing: 0.06em; color: #64748b;
}
...
.rep-input:focus { border-color: var(--rep-color); background: #fff; box-shadow: 0 0 0 3px rgba(217,119,6,0.08); }

/* Branches */
.rep-branches {
  display: flex; flex-direction: column; gap: 2px; background: #f8fafc;
  border: 1px solid #e2e8f0; border-radius: 10px; padding: 6px 4px;
...
.rep-radio-input { display: none; }
.rep-radio-mark {
  width: 16px; height: 16px; border: 1.5px solid #cbd5e1; border-radius: 50%;
  flex-shrink: 0; display: flex; align-items: center; justify-content: center;
  transition: all 150ms; background: #fff;
}
.rep-radio-input:checked + .rep-radio-mark { border-color: var(--rep-color); }
.rep-radio-input:checked + .rep-radio-mark::after {
  content: ""; display: block; width: 8px; height: 8px; border-radius: 50%; background: var(--rep-color);
}
